...
# For legibility in this example, these offset values will be written all on one line, with the current
...
def steps_to_reach_exit(input_file):
    ...
    while step < step_total:
        ...
        current_instruction = steps[step]
        steps[step] = current_instruction + 1
        step = step + current_instruction

    return steps_to_exit


def steps_to_reach_exit_part_2(input_file):
...
if __name__ == '__main__':
    print("It takes %d steps to reach the exit." % steps_to_reach_exit('day_05_input.txt'))
    print("It takes %d steps to reach the exit with the new rule." % steps_to_reach_exit_part_2('day_05_input.txt'))
